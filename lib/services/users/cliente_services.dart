...
//método para verificar permissões antes de realizar operações
  Future<bool> checkPermission(String permission) async {
    if (clienteModel?.usersAccess == null) return false;
    return _accessServices.hasPermission(
        clienteModel!.usersAccess!, permission);
  }
  ...
  Future<bool> signUp2({
    ...
    try {
      ...
      // Atualiza a variável de instância
  ...
  Future<void> signIn({
    ...
    try {
      ...
      if (clienteModel?.usersAccess != null &&
          !_accessServices.isValidAccess(clienteModel!.usersAccess!)) {
        await _auth.signOut();
      ...
      if (clienteModel?.usersAccess != null) {
        _accessServices.updateLastAccess(clienteModel!.usersAccess!);
        await saveUserDetails(); // Salva a atualização do último acesso
      }
  ...
  bool hasPageAccess(String page) {
    if (clienteModel?.usersAccess == null) return false;
    return _accessServices.hasPageAccess(clienteModel!.usersAccess!, page);
  }
